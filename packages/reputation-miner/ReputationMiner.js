...
class ReputationMiner {
  ...
  async addSingleReputationUpdate(updateNumber, repCycle, blockNumber) {
    ...
    const [colonyAddress, skillId, userAddress] = ReputationMiner.breakKeyInToElements(key);
  ...
  async saveCurrentState() {
    ...
    for (let i = 0; i < Object.keys(this.reputations).length; i += 1) {
      ...
      let [colonyAddress, skillId, userAddress] = ReputationMiner.breakKeyInToElements(key); // eslint-disable-line no-await-in-loop
